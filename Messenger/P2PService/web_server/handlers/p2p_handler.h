                       ...
                       HTTPServerResponse &response)
    {
        ...
        try
        {
            if (hasSubstr(request.getURI(), "/send_message") &&
                ...
                form.has("receiver_id") &&
                form.has("text"))
            {
                long sender_id = cur_user_id;

                long receiver_id = atol(form.get("receiver_id").c_str());
                std::string text = form.get("text").c_str();

                database::P2P::send_p2p(sender_id,receiver_id,text);
            ...
            {
                long sender_id = atol(form.get("id").c_str());

                long receiver_id = cur_user_id;

                auto results = database::P2P::get_p2p_messages(sender_id, receiver_id);
                Poco::JSON::Array arr;
                for (auto s : results)
                    arr.add(s.toJSON());
                response.setStatus(Poco::Net::HTTPResponse::HTTP_OK);
